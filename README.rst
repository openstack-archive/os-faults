...
Service-based query performs specified `action` against `service` on
all or selected nodes::

  <action> <service> service [on (random|one|single|<fqdn> node[s])]

Node-based query performs specified `action` on all or selected service's
node::

  <action> [random|one|single] <service> node[s]

Network-management query is a subset of node-based query::
 ...
 * `kill nova-api service on one node` - restarts Nova service on one of nodes
