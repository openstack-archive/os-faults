...
for NODE in `ssh -i ${KEY_FILE_NAME} -o UserKnownHostsFile=/dev/null -o StrictHostKeyChecking=no ${USERNAME}@${HOST} kubectl get nodes -o jsonpath='{.items[*].metadata.name}'`; do
