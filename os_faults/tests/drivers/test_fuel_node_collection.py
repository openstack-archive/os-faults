...
class FuelNodeCollectionTestCase(test.TestCase):
    ...
    def setUp(self):
        ...
        self.hosts = [
            dict(ip='10.0.0.2', mac='09:7b:74:90:63:c1',
                 fqdn='node1.com', roles=['controller']),
            dict(ip='10.0.0.3', mac='09:7b:74:90:63:c2',
                 fqdn='node2.com', roles=['controller']),
            dict(ip='10.0.0.4', mac='09:7b:74:90:63:c3',
                 fqdn='node3.com', roles=['controller']),
            dict(ip='10.0.0.5', mac='09:7b:74:90:63:c4',
                 fqdn='node4.com', roles=['compute']),
        ]
    ...
    def test_filter_computes(self):
        computes = self.node_collection.filter(role='compute')
        self.assertEqual(1, len(computes))
        self.assertEqual([self.hosts[3]], computes.hosts)

    def test_filter_controllers(self):
        controllers = self.node_collection.filter(role='controller')
        self.assertEqual(3, len(controllers))
        self.assertEqual(self.hosts[:3], controllers.hosts)

    def test_filter_empty(self):
        empty = self.node_collection.filter(role='unknown')
