...
class CmdTestCase(test.TestCase):
    ...
    @mock.patch('sys.argv', ['', '-c', 'my_file', 'my_command'])
    def test_human_api_called(self, mock_connect, mock_human_api):
        cmd.main()
        mock_connect.assert_called_once_with(config_filename='my_file')
        mock_human_api.assert_called_once()
        mock_human_api.accert_has_calls(mock.call(
            mock.MagicMock(name='connect()'), 'my_command'))

    @mock.patch('os_faults.human_api')
    @mock.patch('os_faults.connect')
    @mock.patch('sys.argv', ['', '-c', 'my_file', '-v'])
    def test_human_verify(self, mock_connect, mock_human_api):
        cmd.main()
